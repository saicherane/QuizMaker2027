...
const expectedTestFiles = [...authExpectedTestFiles, ...mcqExpectedTestFiles];

describe("test suite inventory", () => {
	it("includes all expected auth phase 1-4 test files", () => {
		for (const relativePath of authExpectedTestFiles) {
			expect(existsSync(join(process.cwd(), relativePath))).toBe(true);
		}
	});

	it("includes all expected MCQ phase 1-4 test files", () => {
		for (const relativePath of mcqExpectedTestFiles) {
			expect(existsSync(join(process.cwd(), relativePath))).toBe(true);
		}
	});
